...
class VaultClient:
    ...
    async def login(self) -> str:
        ...
        try:
            ...
            logger.error(f"[Vault] Login request failed: {e.response.status_code} {e.response.text[:500]}")
            raise
    ...
    async def _request(self, method: str, url: str, **kwargs) -> dict:
        ...
        try:
            ...
            if resp.status_code >= 400:
                logger.error(
                    f"[Vault] {method} {url} returned {resp.status_code}: {resp.text[:500]}"
                )
            ...
            if resp.status_code == 401:
                ...
                if resp.status_code >= 400:
                    logger.error(
                        f"[Vault] {method} {url} (retry) returned {resp.status_code}: {resp.text[:500]}"
                    )
            ...
            logger.error(f"[Vault] Request failed ({method} {url}): {e.response.status_code} {e.response.text[:500]}")
            raise
        except Exception as e:
            logger.error(f"[Vault] Unexpected error in _request ({method} {url}): {str(e)}")
            raise
    ...
    async def get_resource(self, resource_id: str) -> dict:
        ...
        try:
            ...
            if e.response.status_code == 404:
                logger.error(f"[Vault] Resource not found: {resource_id}")
            raise
    ...
    async def download_resource(self, resource_id: str) -> bytes:
        ...
        try:
            ...
            if not download_url:
                logger.error(f"[Vault] No download URL found in response for {resource_id}: {data}")
                raise ValueError(f"Could not find download URL in response: {data}")

            logger.info(f"[Vault] Got download URL, fetching bytes...")
            
            # Fetch the actual bytes
            resp = await self._client.get(download_url)
            resp.raise_for_status()
            logger.info(f"[Vault] Downloaded {len(resp.content)} bytes for {resource_id}")
            return resp.content
            
        except httpx.HTTPStatusError as e:
            logger.error(
                f"[Vault] Download request failed for {resource_id}: "
                f"{e.response.status_code} {e.response.text[:500]}"
            )
            raise
        except Exception as e:
            logger.error(f"[Vault] Download failed for {resource_id}: {str(e)}")
            raise
